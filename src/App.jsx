...
const productFromRow = (row) => ({ ...row, lastUpdated: new Date(row.last_updated).getTime(), imageUrl: row.image_url, expiryDate: row.expiry_date, barcode: row.barcode });
...
function BarcodeScanModal({ onClose, onDetected, subtitle }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/90 flex flex-col items-center justify-center p-6">
      ...
      <button onClick={onClose} className="mt-3 bg-white/20 text-white px-6 py-2 rounded-full text-sm">Cancel</button>
    </div>
  );
}
...
function MyShopDashboard({ shops, onBack, onUpdatePrice, onAddProduct, onOpenSell, onOpenReport, onAddAnotherShop, onDeleteShop }) {
  ...
  const submitNewProduct = async () => {
    ...
    await onAddProduct(newProd.shopId, newProd.name, Number(newProd.price), newProd.unit, Number(newProd.stock), imageUrl, newProd.expiryDate || null, newProd.code || null);
...
function SellFlow({ shop, onBack, onCompleteSale }) {
  ...
  const handleScanResult = (item) => {
    setShowScan(false);
    const match = shop.products.find((p) => p.barcode && p.barcode === item.code);
    if (match) { pickProduct(match); return; }
    alert("Ye barcode kisi product se match nahi hua. List se chuniye, ya product add karte waqt uska barcode scan kar ke save kariye.");
    setShowPicker(true);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-32">
      ...
      {showScan && (
        <BarcodeScanModal subtitle="Bikne wale item ko scan kariye" onClose={() => setShowScan(false)} onDetected={handleScanResult} />
      )}
    ...
    <div className="max-w-md mx-auto bg-gray-50 min-h-screen font-sans relative">
      ...
      {screen === "myShop" && (
        <MyShopDashboard
          ...
          onAddProduct={async (shopId, name, price, unit, stock, imageUrl, expiryDate, barcode) => {
            const { data } = await supabase.from("products").insert({ shop_id: shopId, name, price, unit, stock, history: [{ date: "Today", price }], image_url: imageUrl || null, expiry_date: expiryDate || null, barcode: barcode || null }).select().single();
