...
import { parseProductVoice, splitProductUtterances, parseSaleVoice } from "./lib/voiceParser";
...
function MyShopDashboard({ shops, onBack, onUpdateProduct, onAddProduct, onOpenSell, onOpenReport, onAddAnotherShop, onDeleteShop }) {
  const [newProd, setNewProd] = useState({ shopId: shops[0]?.id, name: "", price: "", code: "", unit: "piece", stock: "", expiryDate: "", packSize: "" });
  ...
  const applyVoiceProduct = (text) => {
    setVoiceText(text);
    const chunks = splitProductUtterances(text);
    const parsed = (chunks.length ? chunks : [text]).map(parseProductVoice).filter(Boolean);
    if (!parsed.length) { alert("Product details samajh nahi aaye. Example: 'Paracetamol 650 price 30, stock 10 patta, expiry 23 January 2030, strip me 10 tablets'."); return; }
    setPending((prev) => [...prev, ...parsed.map((x) => ({ ...x, shopId: newProd.shopId }))]);
  };

  const handleScan = (item) => {
    setShowScan(false);
    setNewProd((p) => ({ ...p, code: item.code || "", name: item.name || p.name, price: item.suggestedPrice || p.price }));
  };

  const addCurrentToReview = () => {
    if (!newProd.name.trim()) return alert("Product naam daaliye.");
    if (!newProd.price || !newProd.stock) return alert("Price aur stock bhi daaliye.");
    setPending((prev) => [...prev, { ...newProd, price: Number(newProd.price), stock: Number(newProd.stock), packSize: newProd.unit === "pack" ? Number(newProd.packSize || 10) : null }]);
    setNewProd((p) => ({ ...p, name: "", price: "", code: "", stock: "", expiryDate: "", packSize: "" }));
  };
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      if (imageFile) imageUrl = await uploadImage(editing.shopId, imageFile);
      const saved = await onUpdateProduct(editing.shopId, editing.id, { ...editing, price: Number(editing.price), stock: Number(editing.stock), imageUrl, expiryDate: editing.expiryDate || null, barcode: editing.barcode || null, unit: editing.unit || "piece" });
      if (editing.unit === "pack") setInventoryMeta(editing.id, { packSize: Number(editing.packSize || 10) });
  ...
  return <div className="pb-24 min-h-screen bg-gray-50">
    ...
    <div className="px-5 mt-4 space-y-4">
      ...
      <div className="bg-white rounded-2xl p-4 border border-gray-100">
        <div className="font-bold text-sm mb-1">Smart Product Add</div><div className="text-xs text-gray-400 mb-3">Manual, barcode ya voice — pehle review, phir ek saath Add.</div>
        <div className="flex gap-2 mb-3"><VoiceInput continuous onText={applyVoiceProduct}/> <button onClick={()=>setShowScan(true)} className="flex items-center gap-1 bg-indigo-50 text-indigo-600 text-xs font-bold px-3 py-2 rounded-xl"><ScanLine size={14}/> Barcode</button></div>
        {voiceText && <div className="text-[11px] bg-violet-50 text-violet-700 rounded-xl p-2 mb-2">Heard: {voiceText}</div>}
        <select value={newProd.shopId} onChange={e=>setNewProd({...newProd,shopId:e.target.value})} className="w-full border rounded-xl px-3 py-2 text-sm mb-2">{shops.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select>
        <input value={newProd.name} onChange={e=>setNewProd({...newProd,name:e.target.value})} placeholder="Product naam" className="w-full border rounded-xl px-3 py-2 text-sm mb-2"/>
        <div className="grid grid-cols-2 gap-2 mb-2"><select value={newProd.unit} onChange={e=>setNewProd({...newProd,unit:e.target.value})} className="border rounded-xl px-3 py-2 text-sm"><option value="piece">Piece</option><option value="pack">Strip / Packet</option><option value="weight">₹ / kg</option><option value="volume">₹ / litre</option><option value="length">₹ / metre</option></select><input value={newProd.price} onChange={e=>setNewProd({...newProd,price:e.target.value.replace(/[^0-9.]/g,"")})} placeholder={`Price ${unitLabel(newProd.unit)}`} className="border rounded-xl px-3 py-2 text-sm"/></div>
        <div className="grid grid-cols-2 gap-2 mb-2"><input value={newProd.stock} onChange={e=>setNewProd({...newProd,stock:e.target.value.replace(/[^0-9.]/g,"")})} placeholder={newProd.unit==="pack"?"Stock (strips)":"Starting stock"} className="border rounded-xl px-3 py-2 text-sm"/>{newProd.unit==="pack"?<input value={newProd.packSize} onChange={e=>setNewProd({...newProd,packSize:e.target.value.replace(/\D/g,"")})} placeholder="1 strip me tablets" className="border rounded-xl px-3 py-2 text-sm"/>:<input type="date" value={newProd.expiryDate} onChange={e=>setNewProd({...newProd,expiryDate:e.target.value})} className="border rounded-xl px-3 py-2 text-sm"/>}</div>
        <input value={newProd.code} onChange={e=>setNewProd({...newProd,code:e.target.value})} placeholder="Barcode (optional)" className="w-full border rounded-xl px-3 py-2 text-sm mb-2"/>
        <button onClick={addCurrentToReview} className="w-full bg-gray-900 text-white font-bold py-3 rounded-xl text-sm">+ Review me add karo</button>
        {pending.length>0 && <div className="mt-4 border-t pt-3"><div className="flex justify-between items-center mb-2"><div className="font-bold text-sm">Review ({pending.length})</div><button onClick={()=>setPending([])} className="text-xs text-red-500">Clear</button></div>{pending.map((p,i)=><div key={i} className="bg-gray-50 rounded-xl p-3 mb-2"><div className="flex justify-between"><div className="font-semibold text-sm">{p.name}</div><button onClick={()=>setPending(x=>x.filter((_,j)=>j!==i))} className="text-red-400"><Trash2 size={14}/></button></div><div className="text-xs text-gray-500 mt-1">₹{p.price} · {p.stock} {p.unit==='pack'?'strips':p.unit}{p.packSize?` · ${p.packSize} tablets/strip`:''}{p.expiryDate?` · Exp ${p.expiryDate}`:''}</div><button onClick={()=>{setNewProd({...newProd,...p});setPending(x=>x.filter((_,j)=>j!==i))}} className="text-xs text-violet-600 font-bold mt-2">Correct</button></div>)}<button disabled={busy} onClick={commitPending} className="w-full bg-emerald-600 disabled:opacity-50 text-white font-bold py-3 rounded-xl">{busy?'Adding…':`✓ Add ${pending.length} Product(s)`}</button></div>}
      </div>
    </div>
...
function SellFlow({ shop, onBack, onCompleteSale }) {
  ...
  const addLine=(product, qty, subUnit)=>{
    const baseQty=saleToBaseUnits(product,qty,subUnit); const current=cart.find(c=>c.productId===product.id); const nextBase=(current?.baseQty||current?.qty||0)+baseQty;
    if(nextBase>Number(product.stock)+1e-9){alert(`${product.name} ka stock kam hai.`);return;}
    const lineTotal=product.unit==='pack'?+(nextBase*Number(product.price)).toFixed(2):product.unit==='piece'?+(nextBase*Number(product.price)).toFixed(2):+(nextBase*Number(product.price)).toFixed(2);
    let displayQty;
    if(product.unit==='pack') displayQty=subUnit&&/tablet|tab|goli/i.test(subUnit)?`${Math.round(nextBase*(product.packSize||10))} tablets`:`${nextBase} strips`;
    ...
    setCart(prev=>current?prev.map(c=>c.productId===product.id?{...c,baseQty:nextBase,qty:nextBase,displayQty,lineTotal}:c):[...prev,{productId:product.id,name:product.name,unit:product.unit,price:Number(product.price),baseQty:nextBase,qty:nextBase,displayQty,lineTotal,packSize:product.packSize||10}]);
  };
  const handleVoice=(text)=>{setVoiceText(text);const matches=parseSaleVoice(text,shop.products);if(!matches.length){alert('Sale samajh nahi aayi. Example: "5 paracetamol tablets aur 1 strip".');return;}matches.forEach(m=>addLine(m.product,m.qty,m.subUnit));};
  ...
  const finish=async()=>{if(!cart.length)return;setFinishing(true);await onCompleteSale(shop.id,cart);setReceipt({id:genId('bill'),shopName:shop.name,items:cart,total,timestamp:Date.now()});setFinishing(false);};
  ...
  <div className="px-5 mt-4 space-y-2"><div className="bg-white rounded-2xl p-3 border"><div className="text-xs text-gray-500 mb-2">Natural language me sale boliye</div><div className="flex items-center justify-between"><VoiceInput onText={handleVoice}/><button onClick={()=>setShowScan(true)} className="flex items-center gap-1 bg-violet-50 text-violet-700 px-3 py-2 rounded-xl text-xs font-bold"><ScanLine size={15}/> Scan</button></div>{voiceText&&<div className="text-[11px] bg-gray-50 rounded-xl p-2 mt-2">Heard: {voiceText}</div>}</div><button onClick={()=>setShowPicker(true)} className="w-full bg-white border-2 border-dashed border-gray-200 text-gray-600 font-semibold py-3 rounded-2xl"><Package size={17}/> Product list se chuno</button></div>
  <div className="px-5 mt-4 space-y-2">{cart.length===0?<div className="text-center text-gray-400 text-sm mt-8">Cart khali hai. Voice, scan ya list use kariye.</div>:cart.map(c=><div key={c.productId} className="bg-white rounded-2xl p-3 border flex justify-between"><div><div className="font-semibold text-sm">{c.name}</div><div className="text-xs text-gray-400">{c.displayQty} · {formatINR(c.price)}{c.unit==='pack'?'/strip':c.unit==='piece'?'/pc':''}</div></div><div className="flex items-center gap-3"><b className="text-violet-600">{formatINR(c.lineTotal)}</b><button onClick={()=>setCart(x=>x.filter(i=>i.productId!==c.productId))} className="text-red-400"><Trash2 size={16}/></button></div></div>)}</div>
  ...
  {reviewing&&<div className="fixed inset-0 z-50 bg-black/50 flex items-end"><div className="w-full max-w-md mx-auto bg-white rounded-t-3xl p-5 max-h-[80vh] overflow-y-auto"><div className="flex justify-between items-center mb-3"><div className="font-bold">Bill Review</div><button onClick={()=>setReviewing(false)}><X size={18}/></button></div>{cart.map(c=><div key={c.productId} className="flex justify-between py-2 border-b text-sm"><span>{c.name}<small className="block text-gray-400">{c.displayQty}</small></span><b>{formatINR(c.lineTotal)}</b></div>)}<div className="flex justify-between font-extrabold text-violet-600 py-3"><span>Total</span><span>{formatINR(total)}</span></div><div className="text-[11px] text-gray-400 mb-3">Galti ho to item remove karke dobara voice/scan se add kar sakte hain.</div><button disabled={finishing} onClick={async()=>{setReviewing(false);await finish()}} className="w-full bg-emerald-600 text-white font-bold py-3 rounded-xl">{finishing?'Saving…':'Confirm Sale & Update Stock'}</button></div></div>}
      ...
      {screen === "myShop" && (
        <MyShopDashboard
          ...
          onUpdateProduct={async (shopId, productId, changes) => {
            ...
            const dbChanges = { name: changes.name, price: Number(changes.price), unit: changes.unit, stock: Number(changes.stock), history: newHistory, image_url: changes.imageUrl || null, expiry_date: changes.expiryDate || null, barcode: changes.barcode || null, last_updated: new Date().toISOString() };
            ...
            setShops((prev) => prev.map((s) => s.id !== shopId ? s : { ...s, products: s.products.map((p) => p.id !== productId ? p : { ...p, ...changes, price: Number(changes.price), stock: Number(changes.stock), lastUpdated: Date.now(), imageUrl: changes.imageUrl || null, expiryDate: changes.expiryDate || null, barcode: changes.barcode || null }) }));
          ...
          onAddProduct={async (shopId, name, price, unit, stock, imageUrl, expiryDate, barcode, packSize) => {
            const { data, error } = await supabase.from("products").insert({ shop_id: shopId, name, price, unit, stock, history: [{ date: "Today", price }], image_url: imageUrl || null, expiry_date: expiryDate || null, barcode: barcode || null }).select().single();
