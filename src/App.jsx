...
const productFromRow = (row) => ({ ...row, lastUpdated: new Date(row.last_updated).getTime(), imageUrl: row.image_url });
...
function MyShopDashboard({ shops, onBack, onUpdatePrice, onAddProduct, onOpenSell, onOpenReport, onAddAnotherShop, onDeleteShop }) {
  ...
  const submitNewProduct = async () => {
    ...
    await onAddProduct(newProd.shopId, newProd.name, Number(newProd.price), newProd.unit, Number(newProd.stock), imageUrl);
    ...
    <div className="max-w-md mx-auto bg-gray-50 min-h-screen font-sans relative">
      ...
      {screen === "myShop" && (
        <MyShopDashboard
          ...
          onAddProduct={async (shopId, name, price, unit, stock, imageUrl) => {
            const { data } = await supabase.from("products").insert({ shop_id: shopId, name, price, unit, stock, history: [{ date: "Today", price }], image_url: imageUrl || null }).select().single();
      ...
      {screen === "myShopReport" && reportShopId && (
        <ShopReportScreen
          ...
          onQuickDiscount={async (shopId, productName) => {
            const shop = shops.find((s) => s.id === shopId);
            const flashDeal = { plan: "2hr", expiresAt: Date.now() + 1000 * 60 * 120, item: productName, discount: "15% off" };
            await supabase.from("shops").update({ flash_deal: flashDeal }).eq("id", shopId);
            setShops((prev) => prev.map((s) => (s.id === shopId ? { ...s, flashDeal } : s)));
            alert(`₹10 paid (mock). "${productName}" pe Flash Deal live ho gaya!`);
          }}
        />
      )}
